...
| `passwordHash` | string | bcrypt hash，Phase 7 加入 |
...
| 儀表板 | `/dashboard` | 個人總覽、結餘摘要 |
| 群組列表 | `/groups` | 所有群組一覽、新建群組 |
| 群組詳情 | `/groups/:id` | 結餘、成員管理 |
| 新增支出 | `/groups/:id/expense/new` | EQUAL / PERCENTAGE / EXACT |
| 結餘結算 | `/groups/:id/balance` | 最少轉帳次數結算清單 |
| 個人設定 | `/settings` | 個人資料、頭貼（未來）|
...
│   ├── infrastructure/     ← Mongoose、Repository 實作、DI Container、JWT/Bcrypt Service
│   └── interfaces/         ← Express Controller、Route、Middleware
├── frontend/               ← React SPA（Vite）
│   └── src/
│       ├── components/     ← PixelButton、PixelCard 等 UI 元件
│       ├── pages/          ← 7 個頁面
│       ├── hooks/          ← useLocalGroups
│       ├── stores/         ← authStore（Zustand）
│       └── lib/            ← api.ts、queryClient.ts
├── e2e/                    ← Playwright E2E（api.spec.ts）
├── docker-compose.yml      ← MongoDB（Phase 7 加入）
